...
const Routes = (app, folderName, jsonData) => {
    jsonData.forEach(route => {
        ...
        const controllerPath = `../api/${folderName}/controllers/${fileName}`;
        ...
        if (middlewaresAction.length > 0) {
            middlewaresAction.forEach(middlewareAction => {
                ...
                const middlewarePath = `../api/${folderName}/middleware/${MiddlewareFileName}`;
